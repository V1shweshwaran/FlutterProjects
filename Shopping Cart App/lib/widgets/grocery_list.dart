...
class _GroceryListState extends State<GroceryList> {
  ...
  void _deleteItem(GroceryItem item) async {
    final url = Uri.https('shoppingcart-c7cd1-default-rtdb.firebaseio.com',
        'shopping_list/${item.id}.json');

    final response = await http.delete(url);

    if (response.statusCode >= 400) {
      // If the deletion failed, you might want to re-add the item to the list
  ...
  void _loadItems() async {
    final url = Uri.https(
        'shoppingcart-c7cd1-default-rtdb.firebaseio.com', 'shopping_list.json');
